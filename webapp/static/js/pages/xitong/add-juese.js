jQuery(function($){
    ...
    function validRoleName(){
        if($.trim(roleName.val())!="") return true;
        alert("请输入角色名称");
        return false;
    }
    function validRoleDesc(){
    	if($.trim(roleDesc.val())!="") return true;
        alert("请输入角色说明");
        return false;
    }
    function validRolePriviliage(){
    	var rolePrivilieges = document.getElementsByName("rolePriviliege");
    	for (var i = 0; i < rolePrivilieges.length; i++){ 
    		if (rolePrivilieges[i].checked)
    			return true;
        }
    	alert("请选择至少一种角色");
    	return false;
    ...
    addForm.on('submit',function(){
        return validRoleName()&&validRoleDesc()&&validRolePriviliage();
    });

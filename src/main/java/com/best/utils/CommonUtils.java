...
public class CommonUtils {
	...
	public static Boolean checkSessionTimeOut(HttpServletRequest request) throws IOException {
		// 一定要去掉 FIXME
		// User user = new User();
		// user.setLoginName("cocu0989");
		// user.setUserEmail("wenchaokai@gmail.com");
		// user.setUserEnable(1);
		// user.setUserId(1L);
		// user.setUserName("温朝凯");
		// user.setMessageSize(2);
		// request.getSession().setAttribute(Constants.USER_TOKEN_IDENTIFY,
		// user);
	...
	public static int createAsnExcelFile(List<Asn> ecbosses, List<String> titles, List<String> titleNames, int index,
			String baseDir, String key, boolean storeTitle) throws Exception {
		if (storeTitle) {
			...
			for (int i = 0; i < titleNames.size(); i++) {
				sheet.addCell(new Label(i, 0, titleNames.get(i)));
			}
			...
			File parentFile = new File(baseDir);
			File file = new File(parentFile, key + ".xls");
			file.renameTo(new File(parentFile, key + "_1.xls"));
			file = new File(parentFile, key + "_1.xls");
			File newFile = new File(parentFile, key + ".xls");
			Workbook book = Workbook.getWorkbook(file);
			WritableWorkbook wwb = Workbook.createWorkbook(newFile, book);

			int indexSheet = 0;
			WritableSheet sheet = null;
			while (true) {
				sheet = wwb.getSheet(indexSheet++);
				if (sheet.getRows() < 62000)
					break;
			}
			for (Asn asn : ecbosses) {
				for (int i = 0; i < titles.size(); i++) {
					sheet.addCell(new Label(i, index, FieldValueCacheUtils.getFieldValue(titles.get(i), asn).toString()));
				}
				if (sheet.getRows() >= 62000) {
					sheet = wwb.createSheet("数据", indexSheet++);
					for (int i = 0; i < titleNames.size(); i++) {
						sheet.addCell(new Label(i, 0, titleNames.get(i)));
					}
					index = 1;
	...
	public static int createAsnItemExcelFile(List<AsnItem> ecbosses, List<String> titles, List<String> titleNames, int index,
			String baseDir, String key, boolean storeTitle) throws Exception {
		if (storeTitle) {
			File parentFile = new File(baseDir);
			if (!parentFile.exists()) {
			...
			for (AsnItem asnItem : ecbosses) {
				for (int i = 0; i < titles.size(); i++) {
					sheet.addCell(new Label(i, index, FieldValueCacheUtils.getFieldValue(titles.get(i), asnItem).toString()));
				}
				if (sheet.getRows() >= 62000) {
					sheet = wwb.createSheet("数据", indexSheet++);
					for (int i = 0; i < titleNames.size(); i++) {
	...
	public static int createLdoExcelFile(List<Ldo> ecbosses, List<String> titles, List<String> titleNames, int index,
			String baseDir, String key, boolean storeTitle) throws Exception {
		if (storeTitle) {
			File parentFile = new File(baseDir);
			if (!parentFile.exists()) {
			...
			for (Ldo ldo : ecbosses) {
				for (int i = 0; i < titles.size(); i++) {
					sheet.addCell(new Label(i, index, FieldValueCacheUtils.getFieldValue(titles.get(i), ldo).toString()));
				}
				if (sheet.getRows() >= 62000) {
					sheet = wwb.createSheet("数据", indexSheet++);
					for (int i = 0; i < titleNames.size(); i++) {
	...
	public static int createLdoItemExcelFile(List<LdoItem> ecbosses, List<String> titles, List<String> titleNames, int index,
			String baseDir, String key, boolean storeTitle) throws Exception {
		if (storeTitle) {
			File parentFile = new File(baseDir);
			if (!parentFile.exists()) {
			...
			for (LdoItem ldoItem : ecbosses) {
				for (int i = 0; i < titles.size(); i++) {
					sheet.addCell(new Label(i, index, FieldValueCacheUtils.getFieldValue(titles.get(i), ldoItem).toString()));
				}
				if (sheet.getRows() >= 62000) {
					sheet = wwb.createSheet("数据", indexSheet++);
					for (int i = 0; i < titleNames.size(); i++) {
